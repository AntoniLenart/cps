...
# Ustawienie na sztywno zerowych prążków widma
X1r[0] = np.real(Y[0])
X1i[0] = 0
X2r[0] = np.imag(Y[0])
X2i[0] = 0

# Połączenie wektorów otrzymując trans
X1 = X1r+1j*X1i
X2 = X2r+1j*X2i
e_time = time.time()
...
print('Różnica: ', abs(np.mean(X1-X1_fft)))
...
plt.xlabel('Częstotliwość [Hz]')
plt.ylabel('Amplituda')
print('Różnica: ', abs(np.mean(X2-X2_fft)))
plt.show()
